...
public class BookingFaculty extends JFrame {
    ...
    private final User facultyUser; // ✅ store logged-in faculty

    // ✅ Constructor now accepts User object
    public BookingFaculty(User facultyUser) {
        this.facultyUser = facultyUser;

        setTitle("Faculty Booking");
        ...
        idField = new JTextField(facultyUser.getUserId());
        idField.setEditable(false);
        formPanel.add(idField);

        formPanel.add(new JLabel("Faculty Name:"));
        nameField = new JTextField(facultyUser.getFullName());
        nameField.setEditable(false);
        formPanel.add(nameField);

        formPanel.add(new JLabel("Resource Type:"));
        resourceBox = new JComboBox<>(new String[]{"Classroom", "Lab", "Sports Facility"});
    ...
    private void handleBooking() {
        try {
            String id = facultyUser.getUserId();
            String name = facultyUser.getFullName();
            String role = "Faculty";
            ...
            // All faculty bookings now go to admin for approval (including classrooms)
    ...
    private void loadExistingBookings() {
        String[] files = {"classrooms.txt", "labs.txt", "sports.txt"};
    ...
    private void saveBookingToFile(Booking b) {
        String filename = switch (b.resource) {
            case "Lab" -> "labs.txt";
            case "Sports Facility" -> "sports.txt";
            default -> "classrooms.txt";
        };
    ...
    // ✅ Back button: return to same logged-in faculty dashboard
    private void goBackHome() {
        dispose();
        new FacultyDashboard(facultyUser);
    }

    private void showMyBookings() {
        List<Booking> myBookings = new ArrayList<>();
        for (Booking b : bookings) {
            if (b.id.equals(facultyUser.getUserId())) {
                myBookings.add(b);
            }
        }
